...
<head>
    ...
    <link href="https://fonts.googleapis.com/css?family=Roboto:100i,200i,300i,400i" rel="stylesheet">
    <style>
        @font-face {
            font-family: 'neon';
...
<body onload="launch('home')">
    ...
    <script>
        ...
        function converttime(timeago) {
            if (timeago < 60) {
                var timeresult = timeago;
                if (timeresult > 1) {
                    return timeresult + ' secs ago';
                }
                else {
                    return timeresult + ' sec ago';
                }
            }
            else if (timeago < 3600) {
                var timeresult = Math.floor(timeago / 60);
                if (timeresult > 1) {
                    return timeresult + ' mins ago';
                }
                else {
                    return timeresult + ' min ago';
                }
            }
            else if (timeago < 86400) {
                var timeresult = Math.floor(timeago / 3600);
                if (timeresult > 1) {
                    return timeresult + ' hours ago';
                }
                else {
                    return timeresult + ' hour ago';
                }
            }
            else if (timeago < 604800) {
                var timeresult = Math.floor(timeago / 86400);
                if (timeresult > 1) {
                    return timeresult + ' days ago';
                }
                else {
                    return timeresult + ' day ago';
                }
            }
            else if (timeago < 2635200) {
                var timeresult = Math.floor(timeago / 604800);
                if (timeresult > 1) {
                    return timeresult + ' weeks ago';
                }
                else {
                    return timeresult + ' week ago';
                }
            }
            else if (timeago < 31536000) {
                var timeresult = Math.floor(timeago / 2635200);
                if (timeresult > 1) {
                    return timeresult + ' months ago';
                }
                else {
                    return timeresult + ' month ago';
                }
            }
            else if (timeago > 31536000) {
                var timeresult = Math.floor(timeago / 31536000);
                if (timeresult > 1) {
                    return timeresult + ' years ago';
                }
                else {
                    return timeresult + ' year ago';
                }
            }
        }
        ...
        function message() {
            ...
            var gradient = document.createElement("div");
            var results = document.createElement("div");
            searchform.className = 'm-searchform';
            title.className = 'm-title';
            messages.className = 'm-results';
            gradient.className = 'gradient';
            title.innerHTML = 'Inbox';
            searchform.placeholder = 'Search';
            var searchinput = document.createElement("input");
            searchinput.className = 'm-searchinput';
            searchinput.placeholder = 'Search';
            $.post("scripts/getmessage.php", {
                ...
                for (var i = 0; i < obj.length; i++) {
                    ...
                    var from_summary = document.createElement('div');
                    var time_opened = document.createElement('div');
                    if (json.opened == 0) {
                        opened.innerHTML = 'new';
                    }
                    else {
                        opened.innerHTML = 'opened';
                    }
                    fromname.innerHTML = json.from;
                    message.innerHTML = json.message;
                    time.className = 'm-result-time';
                    opened.className = 'm-result-opened';
                    fromname.className = 'm-result-from';
                    message.className = 'm-result-message';
                    resultcont.style.color = 'white';
                    resultcont.className = 'm-result-cont';
                    from_summary.className = 'm-from';
                    time_opened.className = 'm-time';
                    var timeago = Math.floor(Date.now() / 1000) - json.time;
                    time.innerHTML = converttime(timeago);
                    appchild(container, resultcont);
                    appchild(resultcont, from_summary, time_opened);
                    appchild(from_summary, fromname, message);
                    appchild(time_opened, opened, time);
                }
            });
            appchild(container, searchform, title, messages, gradient);
            appchild(searchform, searchinput);
            $('.container').fadeIn(200);
        }
